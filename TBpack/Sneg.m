...
(* 
A few NAMING CONVENTIONS used in this code for function arguments and patterns:
...
(* Interleave lists *)
interleave::usage = "interleave[{a1, a2, ...}, {b1, b2, ..}] returns {a1, b1, a2, b2,...}.";

(* Get sublists with a given key *)
keyselect::usage = "keyselect[list, key] selects sublists with the first element equal to key.";

(* Merge sublists with the same first element (key) *)
(* Below there is another implementation mergebasis[] which does the same! *)
keymerge::usage = "keymerge[list1, list2] merges two lists, so that the sublists with
the same first element are combined.";

orthogbasisop::usage = "orthogbasisop[bz1, bz0] orthogonalizes the basis bz1 with respect
to the original basis bz. Both basis sets must be given in the creation 
operator representation.";

orthogbasisvc::usage = "orthogbasisvc[bz1, bz0] orthogonalizes the basis bz1 with respect
...
CR::usage = "CR is a Sneg constant equal to 0; c[CR] is a creation operator.";
AN::usage = "AN is a Sneg constant equal to 1; c[AN] is an annihilation operator.";
DO::usage = "DO is a Sneg constant equal to 0; c[CR, DO] is a spin-down (creation) operator.";
UP::usage = "UP is a Sneg constant equal to 1; c[CR, UP] is a spin-up (creation) operator.";

PROJ0::usage = "PROJ0 is a Sneg constant equal to 1. It is an argument for projector[] specifying projection to a subspace with zero occupancy.";
PROJUP::usage = "PROJUP is a Sneg constant equal to 2. It is an argument for projector[] specifying projection to a subspace with spin up particle.";
PROJDO::usage = "PROJDO is a Sneg constant equal to 3. It is an argument for projector[] specifying projection to a subspace with spin down particle.";
PROJ2::usage = "PROJ2 is a Sneg constant equal to 4. It is an argument for projector[] specifying projection to a subspace with double occupancy.";
PROJ1::usage = "PROJ1 is a Sneg constant equal to 5. It is an argument for projector[] specifying projection to a subspace with single occupancy.";
PROJ02::usage = "PROJ02 is a Sneg constant equal to 6. It is an argument for projector[] specifying projection to a subspace with either zero or double occupancy.";
...
(** Load functions from other packages **)
    ...
    Module[
       	{
       		...
       		x
                   ...
                   Table[(
                   		h = Length[ss]; x = n;
                       	While[x === ss[[h]], h--; x--];
                       	ss = Join[Take[ss, h - 1],Range[ss[[h]]+1, ss[[h]]+Length[ss]-h+1]]
                         ),
                       {Binomial[n, k]-1}
                   ](* end Table *)
	](* end Module *)
...
{  (* We must define the symbol to be numeric. *)
   (* Setting NumberQ[#] ^= True; is not sufficient! *)
   NumericQ[#] ^= True;
   isnumericQ[#] ^= True;
   Conjugate[#] ^= #;
...
(* To save some typing.. One must be careful not to redefine x,y,z,p and m. *)
(* XXX: Protect[] these symbols?? *)
(* Commented out, Nov 2012 *)
(*
op_?spinQ[i___, x] ^= op[i, SPININDEXx];
op_?spinQ[i___, y] ^= op[i, SPININDEXy];
op_?spinQ[i___, z] ^= op[i, SPININDEXz];
op_?spinQ[i___, p] ^= op[i, SPININDEXp];
op_?spinQ[i___, m] ^= op[i, SPININDEXm];
*)
...
(* isnumericQ[] return True if the expression does not involve operators.
   The default behavior is to check numericity using NumericQ and NumberQ,
   but it is possible to override this function for particular purposes. *)

isnumericQ[z_] := NumericQ[z] || NumberQ[z];
...
(* The alternative version using Distribute shows no speed improvement... *)
(* op[a___, b_Plus, c___] := Distribute[Unevaluated[op[a, b, c]], Plus, op]; *)
};
...
nc[a___, Exp[b1_], b2__, Exp[b3_], c___] /; (b1 == -b3) :=
  Module[{kmt}, 
    ...
    Exp[kmt] nc[a, nc[b2], c] /; NumericQ[kmt]
  ];
...
nc[a___, Exp[b_], c___] /; (NumericQ @ Simplify @ pow[b, 2]) := 
  Module[{t},
    t = Simplify @ pow[b, 2];
    If[t === 0, 
      nc[a, 1 + b, c],
...
isospinxyz[op_?fermionQ[j___], n_:0] := Module[{nam, nambar},
  nam = nambu[op[j], n];
  nambar = conj /@ nam;
...
vc2ops[x:vc[l__]] := If[Length[{l}] != Length[BASIS], 
...
(* XXX: preformance bottleneck for transform1[]!!! *)
...
of expr. *)
snegSeries[f_, expr_, n_:5] := Module[{s, x},
