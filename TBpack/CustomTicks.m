...
TODO: Implement TickLabelStyle (after pattern of MinorTickLabelStyle) with suitable fall-through for minor ticks.
...
BeginPackage["TBpack`CustomTicks`"];
...
(* Constants *)
(* To be used instead of In, Out and All used in standard CustomTicks package. 
This redifinition is done because In, Out and All are symbols with already assigned meanings in Mathematica.*)
Inward::usage = "Constant without any specific value used to set inward direction of the ticks on a given axis.";
Outward::usage = "Constant without any specific value used to set outward direction of the ticks on a given axis.";
Both::usage = "Constant without any specific value used to set both inward and outward directions of the ticks on a given axis.";
...
FixedPointForm[x_?NumberQ,{l_Integer?NonNegative,r_Integer?NonNegative},Opts___?OptionQ]:=Module[
	{
		...
		sx,mx,LeftDigits,RightDigits,SeparatorString,SignChars,PaddingChars,
		NaturalLeftDigits,
...
ResolveTickLength[d_?NumericQ,TickLengthScale_?NumericQ,TickDirection:(Inward|Outward|Both),TickReverse:(True|False)]:=Module[
	{LengthPair},

	(* upgrade single length to {inward, outward} pair *)
	LengthPair = Switch[
						TickDirection,
			  			Inward,{d,0},
			  			Outward,{0,d},
...
ResolveTickLength[{d1_?NumericQ,d2_?NumericQ},TickLengthScale_?NumericQ,TickDirection:(Inward|Outward|Both),TickReverse:(True|False)]:=Module[
	{LengthPair},

	(* ignore TickDirection if given {inward, outward} pair *)
	LengthPair = {d1,d2};
...
Options[LinTicks]={
	...
	MinorTickIndexTransformation->Identity,TickTest->(True&),
	TickLabelTest->(True&),
...
LinTicks[x1_?NumericQ,x2_?NumericQ,Opts___?OptionQ]:=Module[
	{UsedRange,DummyGraphics,TickList,MajorCoordList,MinorCoordList},
